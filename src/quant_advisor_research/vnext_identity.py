"""Clean-slate QAR-N1 identity/index wire codec and pure allocation."""
...
class VNextIdentityError(ValueError):
    """Stable, sanitized clean-slate identity error."""
...
def _validate_index(index: object) -> VNextIdentityIndex:
    if type(index) is not VNextIdentityIndex or index.schema_version != VNEXT_INDEX_SCHEMA:
    ...
    names: dict[str, tuple[str, tuple[str | None, ...]]] = {}
...
def parse_vnext_index(payload: Mapping[str, Any]) -> VNextIdentityIndex:
    try:
        ...
        if set(snapshot) != {"schema_version", "entries"}:
            raise _error("identity_index_invalid")
        if type(snapshot["schema_version"]) is not str or snapshot["schema_version"] != VNEXT_INDEX_SCHEMA:
...
def serialize_vnext_index(index: VNextIdentityIndex) -> str:
    ...
    try:
        return json.dumps(
            {"schema_version": VNEXT_INDEX_SCHEMA, "entries": [binding_payload(binding) for binding in validated.bindings]},
            ensure_ascii=False,
            sort_keys=True,
            separators=(",", ":"),
        ) + "\n"
...
def _check_policy(binding: V3IdentityBinding, requested: RequestedArtifactSet, display: DisplayPlacement, *, exact_mode: bool) -> None:
    attachments_match = (
    ...
    display_match = not exact_mode or (
        binding.display_primary == display.display_primary and binding.display_order == display.display_order
    )
