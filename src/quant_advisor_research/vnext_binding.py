"""Shared full validator for clean-slate vNext identity bindings."""
...
_HTML_PATTERN = re.compile(rf"^{_DATE}-{_CADENCE}-model-recommendations(?:\.variant-{_DIGEST})?\.html$")
...
def _name_digest(
    ...
    *,
    ...
    for pattern in patterns:
        ...
        if match is not None and match.group("as_of") == as_of:
            if "cadence" not in match.groupdict() or match.group("cadence") == cadence:
...
def validate_vnext_binding(entry: object, *, allow_legacy_names: bool = False) -> V3IdentityBinding:
    ...
    if type(entry["semantic_fingerprint_version"]) is not str or entry["semantic_fingerprint_version"] != FINGERPRINT_VERSION:
    ...
    if type(entry["artifact_integrity_version"]) is not str or entry["artifact_integrity_version"] != ARTIFACT_INTEGRITY_VERSION:
