...
def report_filename(report: dict[str, Any]) -> str:
    return f"{report['as_of']}-{slug(report['cadence'])}-advisory.html"


def render_report_html(report: dict[str, Any]) -> str:
    title = f"Quant Advisor {report['cadence'].title()} Review - {report['as_of']}"
    recommendation_cards = []
    for rec in report["recommendations"]:
        risks = "\n".join(f"<li>{html.escape(risk)}</li>" for risk in rec["risks"])
        checklist = "\n".join(f"<li>{html.escape(item)}</li>" for item in rec["review_checklist"])
        refs = "\n".join(
        ...
        recommendation_cards.append(
            ...
            <article class="recommendation">
              <header>
                <h2>{html.escape(rec['symbol'])} <span>{html.escape(rec['action'])}</span></h2>
                <p>{html.escape(rec.get('name') or rec['symbol'])}</p>
              </header>
              <dl>
                <div><dt>Stance</dt><dd>{html.escape(rec['stance'])}</dd></div>
                <div><dt>Style</dt><dd>{html.escape(rec['style'])}</dd></div>
                <div><dt>Conviction</dt><dd>{rec['conviction']}</dd></div>
                <div><dt>Evidence</dt><dd>{rec['evidence_score']}</dd></div>
                <div><dt>Risk</dt><dd>{rec['risk_score']}</dd></div>
              </dl>
              <p class="thesis">{html.escape(rec['thesis'])}</p>
              <h3>Risks</h3>
              <ul>{risks}</ul>
...
<head>
  ...
  <link rel="alternate" type="application/rss+xml" title="Quant Advisor RSS" href="feed.xml">
  <style>
    ...
    .recommendation h2 span {{ font-size: .85rem; color: #57606a; font-weight: 500; margin-left: 8px; }}
  ...
  <main>
    <section class="hero">
      ...
      <div class="meta">
        ...
        <span class="pill">Candidates: {report['summary']['candidate_count']}</span>
      </div>
    </section>
    <section class="policy">
      <strong>Policy boundary:</strong> execution, portfolio allocation, and personalized advice are disabled.
      This report is non-personalized research only.
    </section>
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Quant Advisor Research</title>
  <link rel="alternate" type="application/rss+xml" title="Quant Advisor RSS" href="feed.xml">
  <style>
...
<body>
  <main>
    <h1>Quant Advisor Research</h1>
    <p>Non-personalized, recommendation-only research reports. Execution and portfolio allocation are disabled.</p>
...
def render_feed_xml(reports: list[dict[str, Any]], *, site_url: str, feed_title: str) -> str:
    ...
    ET.SubElement(channel, "description").text = "Recommendation-only QuantStrategyLab advisory research reports."
    for report in sorted(reports, key=lambda item: item["as_of"], reverse=True):
        filename = report_filename(report)
        link = f"{site_url.rstrip('/')}/{quote(filename)}"
        item = ET.SubElement(channel, "item")
        ET.SubElement(item, "title").text = f"{report['as_of']} {report['cadence'].title()} Advisory Review"
        ET.SubElement(item, "link").text = link
        ET.SubElement(item, "guid").text = link
        ET.SubElement(item, "pubDate").text = format_datetime(report["generated_at"])
        top_symbols = ", ".join(report["summary"].get("highest_priority_symbols", []))
        ET.SubElement(item, "description").text = (
            f"Mode={report['mode']}; audience={report['audience_scope']}; top symbols={top_symbols}. "
            "Execution and portfolio allocation are disabled."
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Publish advisory reports as static HTML and RSS.")
    ...
    parser.add_argument("--feed-title", default="Quant Advisor Research")
    return parser
