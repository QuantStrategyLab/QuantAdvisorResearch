...
自适应配置控制面（P0，后续 M1–M3）不是第二个顾投。它只在已经通过 lifecycle
准入、绑定 immutable release 的策略中，结合可验证市场快照和平台对账健康，生成策略/
平台的 Shadow 建议；P0 输出固定为 `no_order=true`、零建议权重。

两者的对象、输入和权限必须保持不同：

| 系统 | 回答的问题 | 可以输出 | 不能输出 |
| --- | --- | --- | --- |
| AssetIdeaAdvisor | 哪些标的、主题值得研究？ | 非个性化研究观点、风险、观察周期、证据 | 仓位、策略切换、平台路由、订单 |
| 自适应配置控制面 | 哪个已批准策略/平台可获得 Shadow 建议？ | 证据化的零仓位 Shadow 排名与拒绝原因 | 新策略、风险上限、运行开启、订单 |
| Risk Gate / 执行层 | 是否在预授权边界内可以执行？ | 确定性允许、拒绝或 reduce-only 结果 | 用新闻或 LLM 叙事绕过限制 |

允许的唯一链路是：顾投报告提出研究假设，经过独立的 P1–P3 数据、回测和证据验证后，
才可能成为 immutable 策略候选，再进入 P0。顾投 artifact 本身不能成为
`qsl.selection_input.v1` 的市场因子、策略基础分、插件风险缩放、平台健康或目标权重，
也不能调用策略切换、runtime target、券商或订单接口。

如果两边观点不一致，Risk Gate、平台对账和 lifecycle 准入优先；顾投结论仅记录为待研究
线索。控制台也应分开显示“研究线索”和“Shadow 运行建议”，不能把相同标的/主题的文字
建议误读为持仓意图或对既有策略的反向验证。
