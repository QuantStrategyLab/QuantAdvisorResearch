...
定时构建只从成功的定时 GitHub Actions artifact 下载事件、观察列表和主题动量输入，并放入 runner 临时目录；手工回填必须显式指定并校验上游 run ID。manifest 绑定上游仓库、workflow run、head SHA、artifact ID/名称、文件名与下载后 SHA-256；没有这组 lineage 的可选输入在定时任务中保持不可用。
